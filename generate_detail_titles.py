...
def extract_action(detail_desc: str):
    ...
    tokens = re.findall(r'[가-힣0-9]+', text)
...
def build_title(row: dict) -> str:
    ...
    if result is None:
        raw_tokens = [strip_particle(tok) for tok in re.findall(r'[가-힣0-9]+', normalize(row.get('rfp_title', '')))]
